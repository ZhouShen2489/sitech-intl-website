...
export default async function HelportProductPage({
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {page.caseStudies.map((item) => (
              <article key={item.title.en} className="interactive-card rounded-[2rem] border border-slate-200 bg-white p-7 shadow-card">
      ...
      <section className="bg-white py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <SectionHeading title={copy(locale, page.advantageTitle)} text={copy(locale, page.advantageText)} />
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {page.advantages.map((item) => (
              <article key={item.title.en} className="interactive-card rounded-[2rem] border border-blue-100 bg-[#f8fbff] p-7 shadow-card">
                <h2 className="font-display text-2xl text-ink">{copy(locale, item.title)}</h2>
                <p className="mt-4 text-sm leading-7 text-slate-600">{copy(locale, item.text)}</p>
              </article>
            ))}
